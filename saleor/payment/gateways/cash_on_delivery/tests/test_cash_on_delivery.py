...
def setup_dummy_gateway(settings):
    settings.PLUGINS = ["saleor.payment.gateways.cash_on_delivery.plugin.DummyGatewayPlugin"]
    return settings
...
def test_authorize_gateway_error(payment_dummy, monkeypatch):
    monkeypatch.setattr("saleor.payment.gateways.cash_on_delivery.dummy_success", lambda: False)
    with pytest.raises(PaymentError):
        txn = gateway.authorize(payment=payment_dummy, token="Fake")
        assert txn.kind == TransactionKind.AUTH
